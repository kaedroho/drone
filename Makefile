...
build:
	...
	go build -o packaging/root/usr/local/bin/drone-flake8  -ldflags "-X main.revision $(SHA) -X main.version $(VERSION)" github.com/drone/drone/cli
	go build -o packaging/root/usr/local/bin/droned-flake8 -ldflags "-X main.revision $(SHA) -X main.version $(VERSION)" github.com/drone/drone/server

install:
	install -t /usr/local/bin packaging/root/usr/local/bin/drone-flake8
	install -t /usr/local/bin packaging/root/usr/local/bin/droned-flake8
